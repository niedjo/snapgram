...
export async function signInAccount(user: { email: string; password: string }) {
    try {
        ...
        // Si un utilisateur est connecté (session active), on ne crée pas une nouvelle session
        if (currentUser) {
        ...
        if (error.code === 401) { // Code d'erreur pour session non active
            try {
                ...
                throw error; // On relance l'erreur pour que le gestionnaire d'erreurs en amont puisse la capturer
            }
        } else {
            // Si une autre erreur se produit, on la logge
...
export async function updatePost(post: IUpdatePost) {
  ...
  try {
    ...
    if(hasFileToUpdate) {
      // Update file to appwrite storage
      const uploadedFile = await uploadFile(post.file[0]);
